...
NVCC_FLAGS = -g -G -Xcompiler -Wall -std c++03 -arch=sm_60	## Requires -arch=sm_60 flag to perform Atomics on double precision arrays
LIBRARIES = -L/usr/local/cuda/lib -lcufft 
INCLUDES = -I/usr/local/cuda/samples/common/inc -I/usr/local/cuda/inc

gnss.exe: main.o solver.o statistics.o cudafuncs.o iofuncs.o initialize.o fftfuncs.o allocate.o deallocate.o
...
deallocate.o: deallocate.cu dnsparams.h deallocate.h struct_def.h
	$(NVCC) $(NVCC_FLAGS) -dc -o $@ $< $(INCLUDES) $(LIBRARIES)

main.o: main.cu dnsparams.h statistics.h cudafuncs.h struct_def.h declare.h allocate.h deallocate.h
	$(NVCC) $(NVCC_FLAGS) -dc -o $@ $< $(INCLUDES) $(LIBRARIES)
